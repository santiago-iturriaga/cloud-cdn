...
public class RandomRouting extends RoutingAlgorithm {
	public RandomRouting(CloudCDN_SO problem) {
		super(problem);
	}
		...
		try {
			...
			for (int i = 0; (i < problem_.getTrafico().size())
				...
				if (arrival >= startTime) {
					...
					if (day == current_day) {
						assigned = false;
						current_dc = PseudoRandom.randInt(0, problem_.getRegionesDatacenters().size()-1);
						best_dc = current_dc;
