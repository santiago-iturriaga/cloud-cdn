 ...
 */
public class HUXCrossoverf201603 extends Crossover {

    /**
     * Valid solution types to apply this operator
     */
    private static final List VALID_TYPES
            = Arrays.asList(CloudCDNSolutionf201603Type.class);
     ...
     */
    public HUXCrossoverf201603(HashMap<String, Object> parameters) {
        super(parameters);
    ...
    public Solution[] doCrossover(double probability, Solution parent1,
        ...
        try {
            if ((parent1.getType().getClass().equals(CloudCDNSolutionf201603Type.class))
                    || (parent2.getType().getClass().equals(CloudCDNSolutionf201603Type.class))) {
                if (PseudoRandom.randDouble() < probability) {
                    {
                        ArrayInt p1 = (ArrayInt) parent1.getDecisionVariables()[0];
                        ArrayInt p2 = (ArrayInt) parent2.getDecisionVariables()[0];

                        for (int index = 0; index < p1.getLength(); index++) {
                            if (p1.getValue(index) != p2.getValue(index)) {
                                if (PseudoRandom.randDouble() < 0.5) {
                                    ((ArrayInt) offSpring[0].getDecisionVariables()[0]).setValue(index, p2.getValue(index));
                                    ((ArrayInt) offSpring[1].getDecisionVariables()[0]).setValue(index, p1.getValue(index));
                                }
                            }
                        }
                    }
                    {
                        Binary p1 = (Binary) parent1.getDecisionVariables()[1];
                        Binary p2 = (Binary) parent2.getDecisionVariables()[1];

                        for (int index = 0; index < p1.getNumberOfBits(); index++) {
                            if (p1.getIth(index) != p2.getIth(index)) {
                                if (PseudoRandom.randDouble() < 0.5) {
                                    ((Binary) offSpring[0].getDecisionVariables()[1]).setIth(index, p2.getIth(index));
                                    ((Binary) offSpring[1].getDecisionVariables()[1]).setIth(index, p1.getIth(index));
                                }
                            }
                        }
                    }
