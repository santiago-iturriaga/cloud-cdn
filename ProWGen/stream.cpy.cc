	 ...
	 if (noofDistinctDocs > 0)
	   {
	     ...
	     fprintf(fp,"%10f %10u %10u\n",currentT, req->GetFileId(),req->GetFileSize());

// This increment should be substututed with a procedure. See below. 
      ...
      totalReqGenerated++;
// This increment should be substututed with a procedure 
      currentT = current_time(currentT);
   }
 ...
 Node* sizeCDF, int noofItems2)
{
 ...
 int corr_num, track_corr; 
 
 printf("%f\n", correlation);   
 corr_num = get_corr(noofDistinctDocs, correlation); 
 printf("%d\n", corr_num); 
 track_corr = 0; 
...
current_time(double time)
  {
    ...
    /* Set up first event arrival */
    srand(12357);
    now = time + Exponential(1.0/ARRIVAL_RATE);
    return(now);
  }

int get_corr(int numdocs, float corr) { 
	int corl; 
	...
	if (corr < 0)  
		corl = (int) (numdocs * corr * (-1)); 
	else 
		corl = (int) (numdocs * corr); 
	printf("%d %f %d\n", numdocs, corr, corl); 
	return(corl); 	
