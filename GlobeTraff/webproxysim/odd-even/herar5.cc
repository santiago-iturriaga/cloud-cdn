...
// The dispatch mechanism is no overlap of requests
// with an odd-even size split across levels of the caching hierarchy
...
int main(int argc, char* argv[])
{
 ...
 while (cache_size < infiniteCache*1024*1024)
  ...
  //This loop simulates a no overlap request stream 
		...
		if (size % 2 == 0) 
		  {
		    if (!child1->simulate(fileid, size))
		      {
			//fprintf(out,"%d %d\n",fileid,size);
			parent->update_statistics(size);
		      }
		  }
		else
		  {
		    child1->update_statistics(size);
fprintf(out,"%d %d\n",fileid,size);
